(function(){
  ...
  // ── SMS thread: separate playIO + rearmIO observers (mandatory per spec 2026-07-24) ──
  ...
  function playThread(){
    ...
    clearTimers();
    resetThread();
    // D4 timing: bubble1=~1.0s, typing1, bubble2=~3.5s, bubble3=~6.5s, typing2, bubble4=~9.0s
    var seq = [
      { t: 300,  action: function(){ bubbles[0].classList.add('show'); } },
      { t: 1100, action: function(){ typers[1].classList.add('show'); } },
      { t: 2600, action: function(){ typers[1].classList.remove('show'); bubbles[1].classList.add('show'); } },
      { t: 4000, action: function(){ bubbles[2].classList.add('show'); } },
      { t: 5100, action: function(){ typers[2].classList.add('show'); } },
      { t: 6400, action: function(){ typers[2].classList.remove('show'); bubbles[3].classList.add('show'); playing = false; } }
    ];
  ...
  // Separate playIO + rearmIO observers: mandatory per spec 2026-07-24
  var _armed = true;
  function _autoplay(){
    if (!_armed) return;
    _armed = false;
  ...
  if ('IntersectionObserver' in window){
    // playIO: fires play when >= 15% visible
    ...
    // rearmIO: SEPARATE observer: resets and re-arms ONLY on full exit
    ...
    // getBoundingClientRect check: play immediately if already in viewport on load
  ...
  // ── Animated stat counter: target=300, prefix="$", suffix=" avg emergency call" ──
  // Generation token pattern per spec (runCount / countRun)
  ...
  function runCount(){
    ...
    function step(ts){
      if (run !== countRun) return; // superseded generation, kill
      if (!start) start = ts;
  ...
  // ── A1: hide sticky CTA bar while real CTA panel is in view ──
